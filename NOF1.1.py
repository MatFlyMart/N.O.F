...
# Definir la función homográfica
def homographic_function(z):
    return (z - 1j) / (z + 1j)
...
z_vals = x_grid + 1j * y_grid

# Aplicar la función homográfica a los valores de z
w_vals = homographic_function(z_vals)

# Extraer partes reales e imaginarias de w
real_part = np.real(w_vals)
imaginary_part = np.imag(w_vals)

# Graficar parte real de w en 3D
fig = plt.figure(figsize=(10, 6))
ax = fig.add_subplot(111, projection='3d')
ax.plot_surface(x_grid, y_grid, real_part, cmap='viridis')
ax.set_xlabel('Parte Real de z')
ax.set_ylabel('Parte Imaginaria de z')
ax.set_zlabel('Parte Real de w')
ax.set_title('Gráfica de la Parte Real de la Función Homográfica')
plt.show()

# Graficar parte imaginaria de w en 3D
fig = plt.figure(figsize=(10, 6))
ax = fig.add_subplot(111, projection='3d')
ax.plot_surface(x_grid, y_grid, imaginary_part, cmap='plasma')
ax.set_xlabel('Parte Real de z')
ax.set_ylabel('Parte Imaginaria de z')
ax.set_zlabel('Parte Imaginaria de w')
ax.set_title('Gráfica de la Parte Imaginaria de la Función Homográfica')
plt.show()
...
# Definir la función homográfica
def homographic_function(y, x):
    return (x + y * 1j - 1j) / (x + y * 1j + 1j)

# Generar valores de z en el primer cuadrante
x_vals = np.linspace(0, 1, 501)
y_vals = np.linspace(0, 1, 501)
x_vals2 = np.linspace(1, 500, 500)
y_vals2 = np.linspace(1, 50, 500)
x_grid, y_grid = np.meshgrid(x_vals, y_vals)
x_grid2, y_grid2 = np.meshgrid(x_vals2, y_vals2)
z_vals = x_grid + 1j * y_grid

# Aplicar la función homográfica a los valores de z
w_vals = homographic_function(x_grid,y_grid)
w_vals2 = homographic_function(x_grid2,y_grid2)

# Graficar en el primer cuadrante
plt.figure(figsize=(8, 6))
plt.scatter(np.real(homographic_function(1,0)), np.imag(homographic_function(1,0)), label = "1,0")
plt.scatter(np.real(homographic_function(0,1)), np.imag(homographic_function(0,1)), label = "0,1")
plt.scatter(np.real(homographic_function(0,0)), np.imag(homographic_function(0,0)), label = "0,0")
plt.scatter(np.real(homographic_function(1,1)), np.imag(homographic_function(2,0)), label = "2,0")
plt.scatter(np.real(homographic_function(2,1)), np.imag(homographic_function(0,2)), label = "0,2")
""" plt.scatter(np.real(w_vals), np.imag(w_vals))
plt.scatter(np.real(w_vals2), np.imag(w_vals2)) """
plt.axhline(0, color='black', linewidth=0.5, linestyle='--')
plt.axvline(0, color='black', linewidth=0.5, linestyle='--')
plt.xlabel('Parte Real')
plt.ylabel('Parte Imaginaria')
plt.title('Gráfica de la Función Homográfica en el Primer Cuadrante')
plt.legend()
plt.grid()
plt.show()
